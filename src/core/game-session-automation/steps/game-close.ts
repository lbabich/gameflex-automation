...
const discover = async (ctx: FullStepContext) => {
  await discovery.discoverTarget(ctx, {
    ...
    verifyClick: stepUtils.onGelEvent(gelEvents.GEL_EVENT.GAME_CLOSE, CLOSE_VERIFY_TIMEOUT_MS),
    checkComplete: stepUtils.gelCheck(gelEvents.GEL_EVENT.GAME_CLOSE),
  });
};
