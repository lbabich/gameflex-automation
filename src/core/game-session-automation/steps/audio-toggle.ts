...
export const discover = async (ctx: SessionContext) => {
  try {
    await discoverTarget(ctx, {
      ...
      verifyClick: async (verifyCtx, x, y) => {
...
export async function run(ctx: SessionContext, cachedSteps: CachedStep[] | null) {
